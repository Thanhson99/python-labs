...
def scrape_algorithm(url):
    ...
    if response.status_code != 200:
        return {"url": url, "code": "Unable to fetch data.", "time": "N/A"}

    soup = BeautifulSoup(response.text, "html.parser")

    # Search for code inside <pre>, <code>, or code-like classes
    code_blocks = soup.find_all(["pre", "code"], class_=lambda x: x and "code" in x.lower())

    if not code_blocks:
        return {"url": url, "code": "Code not found; the page may be descriptive only.", "time": "N/A"}
